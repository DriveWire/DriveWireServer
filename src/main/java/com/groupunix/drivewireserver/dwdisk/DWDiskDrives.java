...
public class DWDiskDrives {
    ...
    public DWDiskDrives(DWProtocolHandler dwProto) {
        ...
        for (int i = 0; i < getMaxDrives(); i++) {
            ...
            if (!DriveWireServer.getNoMount() && dwProto.getConfig().getBoolean("RestoreDrivePaths", true) && (dwProto.getConfig().getString("Drive" + i + "Path", null) != null)) {
                ...
                catch (DWImageFormatException | IOException e) {
    ...
    public void writeSector(int driveno, byte[] data) throws DWDriveWriteProtectedException, IOException {
    ...
    public byte[] readSector(int driveno) throws IOException, DWImageFormatException {
        if (dwProto.getConfig().getBoolean("HDBDOSMode", false)) {
            driveno = this.hdbdosdrive;
        }
    ...
    public void seekSector(int driveno, int lsn) throws DWDriveNotLoadedException, DWInvalidSectorException, DWSeekPastEndOfDeviceException {
        if (dwProto.getConfig().getBoolean("HDBDOSMode", false)) {
    ...
    public boolean isDriveNo(int driveno) {
        return (driveno >= 0) && (driveno < getMaxDrives());
    }
    ...
    public void EjectDisk(int driveno) throws DWDriveNotLoadedException {
    ...
    public void LoadDiskFromFile(int driveno, String path) throws IOException, DWImageFormatException {
    ...
    public void LoadDisk(int driveno, DWDisk disk) {
    ...
    public void sync() {
        ...
        for (int driveno = 0; driveno < getMaxDrives(); driveno++) {
            if (isLoaded(driveno)) {
                ...
                catch (DWDriveNotLoadedException | DWDriveNotValidException | IOException e) {
                    logger.warn(e.getMessage());
                }
            }
    ...
    public int nameObjMount(String objname) {
        ...
        catch ( IOException | DWImageFormatException e) {
