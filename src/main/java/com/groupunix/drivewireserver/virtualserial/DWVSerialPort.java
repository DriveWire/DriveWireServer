...
public class DWVSerialPort {
    ...
    private final DWVSerialPorts vports;

    private byte PD_INT = 0;
    private byte PD_QUT = 0;
    private byte[] DD = new byte[26];

    private final DWVSerialCircularBuffer inputBuffer = new DWVSerialCircularBuffer(INPUT_BUFFER_SIZE, true);

    private final byte[] outbuf = new byte[OUTPUT_BUFFER_SIZE];
    private final ByteBuffer outputBuffer = ByteBuffer.wrap(outbuf);
