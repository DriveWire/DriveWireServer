...
public class DWLogAppender extends AppenderSkeleton {
    ...
    protected void append(LoggingEvent event) {
        ...
        if ((null != event) && (event.getMessage() != null) && (event.getLocationInformation().getClassName() != null) && (!event.getMessage().equals("ConfigurationUtils.locate(): base is null, name is null")) && (!event.getLocationInformation().getClassName().startsWith("org.apache.commons.httpclient"))) {
    ...
    public ArrayList<String> getLastEvents(int num) {
        ArrayList<String> eventstxt = new ArrayList<>();
        int start = 0;
