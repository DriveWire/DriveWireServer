...
public class DWVModem {
    ...
    // modem state
    private final int[] vmodem_registers = new int[256];
    private String vmodem_lastcommand = "";
    private String vmodem_dialstring = "";
                ...
                if (dialing) {
                    ...
                    switch (cmd.toUpperCase().charAt(i)) {
                        ...
                        case 'A':
                            ...
                            if (extendedpart) {
                                thiscmd.append(cmd, i, i + 1);
                            }
                            else {
                                // terminate existing command if any
                                if (!(thiscmd.length() == 0)) {
                                    errors += doCommand(thiscmd.toString(), thisreg.toString(), thisarg.toString());
                                }
            ...
            case 'I':
                switch (val) {
                    ...
                    case 2:
                        ...
                        catch (DWPortNotValidException | DWConnectionNotValidException e) {
    ...
    private void enableListener(int val) {
        Thread listenThread = new Thread(new DWVModemListenerThread(this));
        listenThread.start();
    }
    ...
    private int doDial() {
        ...
        Thread tcpthread = new Thread(new DWVModemConnThread(this, tcphost, tcpport));
    ...
    public boolean isEcho() {
        return this.vmodem_registers[REG_ECHO] == 1;

    }

    public boolean isVerbose() {
        return this.vmodem_registers[REG_VERBOSE] == 1;

    }

    public boolean isQuiet() {
        return this.vmodem_registers[REG_QUIET] == 1;
