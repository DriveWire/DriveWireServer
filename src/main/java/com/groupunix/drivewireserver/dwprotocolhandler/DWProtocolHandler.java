...
public class DWProtocolHandler implements Runnable, DWVSerialProtocol {
    ...
    private void DoOP_READ(int opcode) throws IOException, DWCommTimeOutException {
        ...
        catch (DWInvalidSectorException | DWImageFormatException | DWSeekPastEndOfDeviceException e5) {
        ...
        if (result == DWDefs.DWOK) {
            ...
            mysum[1] = (byte) (lastChecksum & 0xFF);
    ...
    private void DoOP_SERINIT() throws IOException, DWCommTimeOutException {
        byte[] responsebuf = new byte[2];
    ...
    private void DoOP_SERTERM() throws IOException, DWCommTimeOutException {
        int portnum;
    ...
    private void DoOP_SERREAD() {
        ...
        if (config.getBoolean("LogOpCodePolls", false)) {
            logger.info("DoOP_SERREAD response " + (result[0] & 0xFF) + ":" + (result[1] & 0xFF));
        }
    }
    ...
    private void DoOP_SERREADM() throws IOException, DWCommTimeOutException {
        ...
        try {
            ...
            protodev.comWrite(data, cmdpacket[1] & 0xFF, true);
