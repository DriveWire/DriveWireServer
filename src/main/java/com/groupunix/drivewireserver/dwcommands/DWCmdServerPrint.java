...
public class DWCmdServerPrint extends DWCommand {
    ...
    DWCmdServerPrint(DWProtocol dwProto, DWCommand parent) {
        setParentCmd(parent);
    ...
    static void closeObjects(InputStream ins, FileObject fileobj, FileContent fc) {
        try {
            if (ins != null) {
                ins.close();
            }
